  ...
  return (
    <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        ...
        <button className="btn btn-ghost btn-sm" onClick={openAdd} style={{ fontSize: 12 }}>
          + Add contact
        </button>
      </div>
      <p style={{ fontSize: 14, color: 'var(--slate)', marginBottom: 16, lineHeight: 1.6 }}>
        People you can assign for drop-off and pick-up — family, grandparents, carpool friends.
      </p>

      {loading ? (
        <div className="spinner" style={{ width: 20, height: 20 }} />
      ) : contacts.length === 0 ? (
        <div style={{ background: 'var(--off-white)', borderRadius: 8, padding: '20px',
                      textAlign: 'center', color: 'var(--slate)', fontSize: 14 }}>
          No contacts yet. Add grandparents, carpool friends, or anyone who helps with rides.
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {contacts.map(c => (
            <div key={c.id} style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between',
              padding: '12px 16px', background: 'var(--off-white)', borderRadius: 8,
            }}>
              <div>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <button className="btn btn-ghost btn-sm" onClick={() => handleDelete(c.id)}
                  style={{ fontSize: 11, color: '#ef4444' }}>Remove</button>
              </div>
            </div>
          ))}
        </div>
